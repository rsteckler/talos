...
export function runMigrations(): void {
  ...
  const currentVersion = (raw.prepare(
    "SELECT MAX(version) as v FROM schema_version"
  ).get() as { v: number | null })?.v ?? 0;

  function applyMigration(version: number, fn: () => void): void {
    if (currentVersion >= version) return;
    fn();
    raw.prepare("INSERT INTO schema_version (version, applied_at) VALUES (?, ?)")
      .run(version, new Date().toISOString());
  }

  // --- v1: Baseline (all existing CREATE TABLE IF NOT EXISTS + ad-hoc ALTERs) ---
  applyMigration(1, () => {
    raw.exec(`
  ...
  // --- v2: Rename tool_configs → plugin_configs ---
  applyMigration(2, () => {
    // Only rename if old table exists and new one doesn't
    const oldExists = raw.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='tool_configs'"
    ).get();
    if (oldExists) {
      raw.exec("ALTER TABLE tool_configs RENAME TO plugin_configs;");
      raw.exec("ALTER TABLE plugin_configs RENAME COLUMN tool_id TO plugin_id;");
    } else {
      // Fresh install or already renamed — ensure the table exists
      raw.exec(`
  ...
  // --- v3: Add tool_calls column to messages ---
  applyMigration(3, () => {
