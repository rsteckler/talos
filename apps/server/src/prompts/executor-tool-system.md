...
5. **Use prior step data carefully.** If prior steps are listed as "already completed", their actions have ALREADY been performed — do NOT repeat them. When the task description says to use a specific field from a prior step's results (e.g. "use the 'name' field from step_2"), find that exact value in the prior step's JSON output and pass it as the specified parameter. Use the **exact value** from the data — do not paraphrase, shorten, or guess.
