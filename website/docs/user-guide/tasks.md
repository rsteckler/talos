...
Tasks are automated jobs that Talos executes on a schedule or on demand. Each task has a prompt that is sent to the LLM, and optionally a set of plugins it can use.
...
Plugins can declare their own trigger types for event-driven tasks. These appear in the task creation dialog under **Plugin Events** when the plugin is enabled.

For example, the Google Workspace plugin provides a **New Gmail received** trigger (`google:gmail_new_email`) that polls your inbox for new emails. When new mail arrives, any tasks using this trigger fire automatically with the email details included in the prompt.

Plugin triggers:
- Only poll when at least one active task uses them
- Have configurable poll intervals (set in **Settings → Plugins**)
...
4. Optionally select which plugins the task can use
...
1. The action prompt is sent to the LLM with the selected plugins
2. The LLM may make tool calls as part of its response
...
- **Trigger type** — Select from built-in triggers or plugin-provided triggers (grouped in the dropdown)
- For plugin triggers, an info note explains that the trigger fires automatically based on plugin settings
